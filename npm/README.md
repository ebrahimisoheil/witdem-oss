...
```bash
npx -y witdem@0.2.6 status
npx -y witdem@0.2.6 logs
npx -y witdem@0.2.6 down
```

Run `npx -y witdem@0.2.6 --help` for port and image overrides. Docker with the Compose plugin is the only runtime prerequisite. Docker reuses a matching local image and pulls it when missing.
