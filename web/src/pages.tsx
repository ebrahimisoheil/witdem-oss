    ...
    <div className="witdem-overview-page">
      ...
      {d && assurance ? <div className="grid gap-4 xl:grid-cols-[1.05fr_.72fr_.48fr]">
        ...
        <section className="witdem-spend-card flex min-w-0 flex-col rounded-2xl border border-[#ded7f3] bg-[#f8f5ff] p-6 shadow-[0_8px_30px_rgba(62,42,112,.06)]">
          <a href={healthHref} className="text-xs font-semibold uppercase tracking-[.14em] text-[#7151cc]">{costIncomplete ? "Known subtotal" : "Measured spend"}</a>
          <a href={healthHref} className="mt-3 block break-words text-4xl font-semibold tracking-[-.04em] text-[#2f2450]">
      ...
      {models.data ? <div className="mt-4 grid gap-4 xl:grid-cols-2">
        ...
        <Panel className="h-full" title="Model call latency distribution" titleHref={healthHref} note="Direct model-call p50 and p95 latency. Select a model to inspect System Health.">
          <LatencyVariabilityChart items={models.data!.items} onSelect={(item) => window.location.assign(drilldownHref("/system-health", { ...routeBase, model: item.label }))} />
        </Panel>
